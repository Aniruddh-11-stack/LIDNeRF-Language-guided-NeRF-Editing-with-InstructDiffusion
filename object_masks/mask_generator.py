...
class OTMask:
    ...
    def __init__(self, panoptic_metadata, object_mask_path):
        ...
        if self._panoptic:
            try:
                ...
                with open('/home/intel-user1/InstructDiffusion/object_masks/_panoptic_metadata.txt', 'r') as f:
    ...
    def _get_fg_overlapped(self, image_label, image_label_anno, comod_mask, ratio=0.5):
        ...
        # print(comod_mask.shape)
        # print(image_label[0,:,:].shape)
        image_label_inside = image_label[0,:,:,0]*(1-comod_mask) + (image_label[0,:,:,0]*0 + 255)*comod_mask
        
        print(image_label_inside)
        
        labels, counts = np.unique(image_label[0,:,:,0], return_counts=True)
        ...
        fg_to_exclude = np.isin(image_label[0,:,:,0], label_to_exclude)
        

        fg_to_exclude = np.logical_not(np.isin(image_label[0,:,:,0], label_to_exclude))
...
if __name__=="__main__":
    object_mask_loc = '/home/intel-user1/InstructDiffusion/object_masks/'
    image_fname = object_mask_loc+'Places365_test_00000002.jpg'
    anno_seg_fname = object_mask_loc+'Places365_test_00000002_1.jpg'
    anno_json_fname = object_mask_loc+'Places365_test_00000002.json'
    
    mask_fname = object_mask_loc+'output_mask.png'
    masked_image_fname = object_mask_loc+'output_masked_image.png'

    mask_generator = OTMask(panoptic_metadata='/home/intel-user1/InstructDiffusion/object_masks/_panoptic_metadata.txt', object_mask_path='/home/intel-user1/InstructDiffusion/object_masks/')
    # print(np.array(Im.open(image_fname)).shape)
    # print(np.array(Im.open(anno_seg_fname)).shape)
    # print(anno_json_fname.shape)
